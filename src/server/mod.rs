...
use crate::{
    ...
    engine::{Analysis, EngineError, Nodes, PositionEngine, UciEngine},
    fetch, patterns,
    pipeline::{self, Input},
...
async fn create(
        ...
        .is_some_and(|v| v == "cross-site")
    {
        return Err(ApiError(
            ...
            "Open Tempo directly to start a review.".into(),
        ));
    }
...
async fn recent(State(app): State<Arc<App>>) -> ApiResult<Value> {
    let jobs: Vec<_> = app.store.recent()?.iter().map(|j| json!({"id":j.id,"username":j.username,"pace":j.pace,"status":j.status,"created_at":j.created_at,"progress":j.progress})).collect();
    Ok(Json(json!({"reviews":jobs})))
}
...
async fn health(State(app): State<Arc<App>>) -> ApiResult<Value> {
    Ok(Json(
        json!({"status":"ok","engine":app.engine_version,"runtime":"native","worker":"single","persistence":"sqlite"}),
    ))
}
...
impl PositionEngine for ReportingEngine<'_> {
    fn analyse(
        ...
        let result = self.inner.analyse(position, user, nodes)?;
        self.positions += 1;
